...
const measureExecutionTime = process.env.MEASURE_EXECUTION_TIME === 'true';
...
test.afterEach(async ({ page }, testInfo) => {
  if (measureExecutionTime) {
    ...
    if (fs.existsSync('test_results.json')) {
      const existingData = fs.readFileSync('test_results.json', 'utf8');
    ...
    fs.writeFileSync('test_results.json', JSON.stringify(jsonData, null, 2));

    console.log(`Test result appended to test_results.json`);
  }
});
